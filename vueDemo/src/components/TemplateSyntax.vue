...
<template>
    ...
    <p>使用文本插值：{{ rawHtml }}</p>
    <p>使用 v-show 指令：<span v-show="ok">ok is {{ ok }}</span><button @click="ok = !ok">Toggle</button></p>
    <p>使用 v-if 指令：<span v-if="ok">Vue is awesome!</span><span v-else>Oh no 😢</span><button
            @click="ok = !ok">Toggle</button></p>
    <p>使用 v-for 指令：
